...
const GradeDistribution3 = ({
  ...
  }, [selectedEll, selectedSpecialEd, selectedArd, selectedGender, syncDataWithFilters]);
  ...
  const columnDefs = useMemo(
    () => [
      { field: "sc", headerName: "School", filter: true, sortable: true, floatingFilter: true, flex: 2, pivot: true, enableRowGroup: true },
      { field: "tn", headerName: "Teacher Number", filter: true, sortable: true, floatingFilter: true, flex: 2, pivot: true, enableRowGroup: true, hide: true },
      { field: "teacherName", headerName: "Teacher", cellRenderer: createTeacherCellRenderer(), filter: true, sortable: true, floatingFilter: true, flex: 2, pivot: true, enableRowGroup: true },
      { field: "department", headerName: "Department", sortable: true, filter: true, floatingFilter: true, flex: 1, pivot: true, enableRowGroup: true },
      { field: "term", headerName: "Term", sortable: true, filter: true, floatingFilter: true, flex: 1, pivot: true, enableRowGroup: true },
      { field: "courseTitle", headerName: "Course", sortable: true, filter: true, floatingFilter: true, flex: 1.5, pivot: true, enableRowGroup: true },
      { field: "aPercent", headerName: "A%", type: "numericColumn", cellRenderer: PercentCellRenderer, valueFormatter: (params) => params.value ? params.value.toFixed(1) : "", filter: "agNumberColumnFilter", floatingFilter: true, flex: 1, pivot: true, enableRowGroup: true, enableValue: true },
      { field: "bPercent", headerName: "B%", type: "numericColumn", cellRenderer: PercentCellRenderer, valueFormatter: (params) => params.value ? params.value.toFixed(1) : "", filter: "agNumberColumnFilter", floatingFilter: true, flex: 1, pivot: true, enableRowGroup: true, enableValue: true },
      { field: "cPercent", headerName: "C%", type: "numericColumn", cellRenderer: PercentCellRenderer, valueFormatter: (params) => params.value ? params.value.toFixed(1) : "", filter: "agNumberColumnFilter", floatingFilter: true, flex: 1, pivot: true, enableRowGroup: true, enableValue: true },
      { field: "dPercent", headerName: "D%", type: "numericColumn", cellRenderer: PercentCellRenderer, valueFormatter: (params) => params.value ? params.value.toFixed(1) : "", filter: "agNumberColumnFilter", floatingFilter: true, flex: 1, pivot: true, enableRowGroup: true, enableValue: true },
      { field: "fPercent", headerName: "F%", type: "numericColumn", cellRenderer: PercentCellRenderer, valueFormatter: (params) => params.value ? params.value.toFixed(1) : "", filter: "agNumberColumnFilter", floatingFilter: true, flex: 1, pivot: true, enableRowGroup: true, enableValue: true },
      { field: "otherPercent", headerName: "Other %", type: "numericColumn", cellRenderer: (props) => `${props.value ? props.value.toFixed(1) : 0}%`, filter: "agNumberColumnFilter", flex: 1, pivot: true, enableRowGroup: true, enableValue: true },
      { field: "totalGrades", headerName: "Total", type: "numericColumn", filter: "agNumberColumnFilter", floatingFilter: true, flex: 1, pivot: true, enableRowGroup: true, enableValue: true },
    ],
    [selectedEll, selectedSpecialEd, selectedArd, selectedGender]
  );

  const defaultColDef = useMemo(() => ({ resizable: true, sortable: true, filter: true, floatingFilter: true, minWidth: 100 }), []);
  const onGridSizeChanged = useCallback((params) => { const gridWidth = document.querySelector(".ag-center-cols")?.clientWidth; if (gridWidth) { params.api.sizeColumnsToFit(); } }, []);
  const onGridReady = useCallback((params) => { setGridApi(params.api); params.api.sizeColumnsToFit(); }, []);

  useEffect(() => {
    const handleResize = () => { if (gridApi) { gridApi.sizeColumnsToFit(); } };
  ...
  const resetFilters = useCallback(() => {
    ...
    try {
      ...
      if (chartRef.current && chartRef.current.chart) { chartRef.current.chart.updateData(initialData || []); }
    } catch (error) {
      console.error("Error resetting filters:", error);
    } finally {
      setIsProcessing(false);
    }
  }, [gridApi, initialData, teacherItems, departmentItems, courseTitleItems, termItems, ellItems, specialEdItems, ardItems, genderItems, schoolItems, periodItems]);
    ...
    () => ({
      title: {
        text: `Grade Distribution by Teacher${
          selectedDepartments.length === 1 ? ` - ${selectedDepartments[0]}` : selectedDepartments.length > 1 ? ` - Multiple Departments` : ""
        }${
          selectedCourseTitles.length === 1 ? ` for ${selectedCourseTitles[0]}` : selectedCourseTitles.length > 1 ? ` for Multiple Courses` : ""
        }${
          selectedTerms.length === 1 ? ` (${selectedTerms[0]})` : selectedTerms.length > 1 ? ` (Multiple Terms)` : ""
        }${
          selectedEll.length > 0 || selectedSpecialEd.length > 0 || selectedArd.length > 0 || selectedGender.length > 0
            ? " [" + [
                selectedEll.length > 0 ? `ELL: ${selectedEll.join(', ')}` : "",
                selectedSpecialEd.length > 0 ? `SpEd: ${selectedSpecialEd.join(', ')}` : "",
      ...
      theme: {
        ...
        palette: {
          fills: ["#7F8C8D", "#C0392B", "#E67E22", "#F4D03F", "#5DADE2", "#2E86C1"],
          strokes: ["gray"],
        },
      },
      series: [
        { type: "bar", xKey: "teacherName", yKey: "otherPercent", yName: "Other%", stacked: true, tooltip: { renderer: CustomTooltip } },
        { type: "bar", xKey: "teacherName", yKey: "fPercent", yName: "F%", stacked: true, tooltip: { renderer: CustomTooltip } },
        { type: "bar", xKey: "teacherName", yKey: "dPercent", yName: "D%", stacked: true, tooltip: { renderer: CustomTooltip } },
        { type: "bar", xKey: "teacherName", yKey: "cPercent", yName: "C%", stacked: true, tooltip: { renderer: CustomTooltip } },
        { type: "bar", xKey: "teacherName", yKey: "bPercent", yName: "B%", stacked: true, tooltip: { renderer: CustomTooltip } },
        { type: "bar", xKey: "teacherName", yKey: "aPercent", yName: "A%", stacked: true, tooltip: { renderer: CustomTooltip } },
      ],
      axes: [
        { type: "category", position: "bottom", label: { rotation: 45 } },
        { type: "number", position: "left", title: { text: "Grade Distribution (%)" }, min: 0, max: 100 },
      ],
      legend: { position: "bottom", spacing: 40 },
      padding: { top: 10, right: 20, bottom: 40, left: 40 },
    }),
    [data, baseChartTheme, selectedDepartments, selectedCourseTitles, selectedTerms, selectedEll, selectedSpecialEd, selectedArd, selectedGender, CustomTooltip]
  );
  ...
  const onFilterChanged = useCallback((params) => { updateChartData(params); }, [updateChartData]);
  const onSortChanged = useCallback((params) => { updateChartData(params); }, [updateChartData]);
          ...
          {isLoading ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <MultiDropdownSelector items={filteredTermItems} values={selectedTerms} onChange={setSelectedTerms} placeholder="Select terms" label="Terms" width="w-full" disabled={showLoading} maxDisplayItems={1} singleSelect={true} itemOrder={allTerms} classNameVar={selectedTerms.length === 0 ? "outline outline-red-600 rounded-md" : ""} />
                <MultiDropdownSelector items={filteredSchoolItems} values={selectedSchools} onChange={setSelectedSchools} placeholder="Select schools" label="Schools" width="w-full" disabled={showLoading} maxDisplayItems={2} schoolValues={user.UserSchool} />
                <MultiDropdownSelector items={filteredCourseTitleItems} values={selectedCourseTitles} onChange={setSelectedCourseTitles} placeholder="Select Courses" label="Courses" width="w-full" disabled={showLoading} maxDisplayItems={1} />
              </div>
              <Separator className="my-4" />
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <MultiDropdownSelector items={filteredTeacherItems} values={selectedTeachers} onChange={setSelectedTeachers} placeholder="Select teachers" label="Teachers" width="w-full" disabled={showLoading} maxDisplayItems={2} />
                <MultiDropdownSelector items={filteredDepartmentItems} values={selectedDepartments} onChange={setSelectedDepartments} placeholder="Select departments" label="Departments" width="w-full" disabled={showLoading} maxDisplayItems={1} />
              </div>
              <Separator className="my-4" />
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <MultiDropdownSelector items={filteredGenderItems} values={selectedGender} onChange={setSelectedGender} placeholder="Select Gender" label="Gender" width="w-full" singleSelect={true} disabled={showLoading || filteredGenderItems.length === 0} maxDisplayItems={1} />
                <MultiDropdownSelector items={filteredEllItems} values={selectedEll} onChange={setSelectedEll} placeholder="Select ELL status" label="ELL Status" width="w-full" singleSelect={true} disabled={showLoading || filteredEllItems.length === 0} maxDisplayItems={1} />
                <MultiDropdownSelector items={filteredSpecialEdItems} values={selectedSpecialEd} onChange={setSelectedSpecialEd} placeholder="Select Special Ed status" label="Special Ed Status" width="w-full" singleSelect={true} disabled={showLoading || filteredSpecialEdItems.length === 0} maxDisplayItems={1} />
                <MultiDropdownSelector items={filteredArdItems} values={selectedArd} onChange={setSelectedArd} placeholder="Select Race / Ethnicity" label="Race / Ethnicity" width="w-full" singleSelect={true} disabled={showLoading || filteredArdItems.length === 0} maxDisplayItems={1} />
              </div>
              <div className="mt-4 flex flex-wrap items-center gap-2">
                <div className="ml-auto">
                  <Button onClick={resetFilters} variant="outline" className="ml-2" disabled={showLoading}>
                    {showLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Resetting...</>) : ("Reset Filters")}
                  </Button>
                </div>
              </div>
            </>
          ...
          <Card>
            <CardHeader>
              ...
              <div className="flex items-center mb-4 w-full items-right">
                <ExportChartButton chartRef={chartRef} filename={`grade distribution${selectedCourseTitles.length > 0 ? " - " : ""}${selectedCourseTitles.join("-")} - ${selectedTerms.join("-") || "all"}`} disabled={showLoading || !filteredData.length} />
              </div>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="h-[500px] relative">
                  <AgCharts options={chartOptions} ref={chartRef} style={{ width: "100%", height: "100%" }} />
                </div>
              )}
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
              ...
              <div className="flex justify-between items-center">
                <Button onClick={exportToCSV} className="bg-primary text-primary-foreground hover:bg-primary/90" disabled={showLoading || !data?.length}>
                  {showLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Exporting...</>) : ("Export to CSV")}
                </Button>
              </div>
            </CardHeader>
            <CardContent>
...
export default GradeDistribution3;