...
interface ApiGradeDistributionProps {
  initialFilters?: Record<string, any>;
  studentAttributes?: StudentAttributes;
}

const ApiGradeDistribution = ({
  initialFilters = {},
  studentAttributes = {
    ellOptions: [],
    specialEdOptions: [],
  ...
  const [filterOptions, setFilterOptions] = useState({
    ...
    courseTitles: [],
    terms: [],
    ellOptions: studentAttributes.ellOptions || [],
    specialEdOptions: studentAttributes.specialEdOptions || [],
    ardOptions: studentAttributes.ardOptions || [],
  });
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      const response = await fetch('/api/grade-distribution', {
        method: 'GET',
        headers: {
      ...
      setFilterOptions({
        ...
        terms: options.terms || [],
        ellOptions: options.ellOptions || studentAttributes.ellOptions || [],
        specialEdOptions: options.specialEdOptions || studentAttributes.specialEdOptions || [],
        ardOptions: options.ardOptions || studentAttributes.ardOptions || [],
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const filters = {
      ...
      departmentCode: selectedDepartments.length > 0 
        ? selectedDepartments 
        : undefined,
      courseTitle: selectedCourseTitles.length > 0 
        ? selectedCourseTitles 
        : undefined,
      term: selectedTerms.length > 0 
        ? selectedTerms 
        : undefined,
      ell: selectedEll.length > 0 
        ? selectedEll 
        : undefined,
      specialEd: selectedSpecialEd.length > 0 
        ? selectedSpecialEd 
        : undefined,
      ard: selectedArd.length > 0 
        ? selectedArd 
        : undefined,
    };
  ...
  // Updated chart options configuration
  ...
  const resetFilters = useCallback(() => {
    ...
    try {
      ...
      // Fetch data with initial filters only
      fetchData(initialFilters);
  ...
  }, [gridApi, initialFilters, fetchData]);

  // Transform teacher data for dropdown
  const teacherItems = useMemo(() => {
    return filterOptions.teachers.map(teacher => ({
      id: teacher.id,
      label: teacher.name
    })).sort((a, b) => a.label.localeCompare(b.label));
  }, [filterOptions.teachers]);

  // Transform department data for dropdown
  const departmentItems = useMemo(() => {
    return filterOptions.departments.map(department => ({
      id: department,
      label: department
    })).sort((a, b) => a.label.localeCompare(b.label));
  }, [filterOptions.departments]);

  // Transform course data for dropdown
  const courseItems = useMemo(() => {
    return filterOptions.courseTitles.map(course => ({
      id: course,
      label: course
    })).sort((a, b) => a.label.localeCompare(b.label));
  }, [filterOptions.courseTitles]);

  // Transform term data for dropdown
  const termItems = useMemo(() => {
    return filterOptions.terms.map(term => ({
      id: term,
      label: term
    })).sort((a, b) => {
      // Sort terms in a meaningful way if possible (PRG1, PRG2, etc.)
  ...
  // Transform ELL data for dropdown
  const ellItems = useMemo(() => {
    return filterOptions.ellOptions.map(ell => ({
      id: ell,
      label: ell === 'Y' ? 'Yes' : ell === 'N' ? 'No' : ell
    }));
  }, [filterOptions.ellOptions]);

  // Transform Special Ed data for dropdown
  const specialEdItems = useMemo(() => {
    return filterOptions.specialEdOptions.map(specialEd => ({
      id: specialEd,
      label: specialEd === 'Y' ? 'Yes' : specialEd === 'N' ? 'No' : specialEd
    }));
  }, [filterOptions.specialEdOptions]);

  // Transform ARD data for dropdown
  const ardItems = useMemo(() => {
    return filterOptions.ardOptions.map(ard => ({
      id: ard,
      label: ard === 'Y' ? 'Yes' : ard === 'N' ? 'No' : ard
    }));
  }, [filterOptions.ardOptions]);
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center">
                ...
                <div className="flex flex-wrap gap-1">
                  {selectedTeachers.map(teacherId => {
                    ...
                    return (
                      <Badge key={teacherId.id} variant="outline" className="text-xs py-0">
                        {teacher?.label || teacherId.label}
                      </Badge>
                    );
                  })}
                </div>
            ...
            {selectedCourseTitles.length > 0 && (
              <div className="flex items-center">
                ...
                <div className="flex flex-wrap gap-1">
                  {selectedCourseTitles.map(courseTitleId => {
                    const courseTitles = courseItems.find(c => c.id === courseTitleId);
                    return (
                      <Badge key={courseTitleId} variant="outline" className="text-xs py-0">
                        {courseTitles?.label || courseTitleId}
                      </Badge>
                    );
                  })}
                </div>
          ...
          {isLoading ? (
            ...
            <div className="h-[600px] w-full relative">
              <AgGridReact
                ...
                paginationPageSize={data?.length > 20 ? 20 : data?.length || 10}
                enableCharts={true}
...
export default ApiGradeDistribution;